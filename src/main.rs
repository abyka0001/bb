...
// Структуры для парсинга ответа Google DNS
...
struct State {
    ...
    data_centers: RwLock<Vec<String>>, // Общий список IP для спама
}
...
fn generate_sign(path: &str, nonce: &str, time: &str, data: &str, device_id: &str) -> String {
    // В Python версии порядок: ApiKey + Path + Nonce + Time + Data + Secret
    // Rust версия уже соответствовала этому.
...
// --- ЛОГИКА ДЛЯ ДИСКОРД БОТА (FRIEND REQUESTS) ---
...
async fn send_friend_request(
    ...
    // Для friend requests используем обычные домены (или рандом) как в оригинале
...
// --- ЛОГИКА ДЛЯ FAMILY SPAM ---

async fn cdn_updater(state: Arc<State>) {
    let client = &state.http;
    
    loop {
        // 1. Получаем список IP из Pastebin
        match client.get("https://pastebin.com/raw/JAUiZzvb").send().await {
            Ok(resp) => {
                if let Ok(text) = resp.text().await {
                    ...
                    if !ips.is_empty() {
                        // 2. Делаем запросы ко всем IP одновременно (без лимита)
...
async fn family_spam_task(state: Arc<State>) {
    ...
    loop {
        // Выбираем случайный Data Center IP
        let target_ip = {
        ...
        if let Some(ip) = target_ip {
            if let Some(account) = state.accounts.choose(&mut rng) {
                ...
                let lang_code_full = parts[0]; // e.g., "en_US"
                let lang_code_short = &lang_code_full[0..2]; // e.g., "en"
                ...
                // В Python для DELETE: data пустая.
                // Sign: ...recruit{nonce}{time}9Eu...
                ...
                // ВАЖНО: Подмена Host, так как мы стучимся по IP
                ...
                // parts[0] is code, parts[1..4] are names. 
                // m_type/o_type 1-based index maps to parts[m_type] if parts has 5 elements (0..4)
                ...
                let mut headers = HeaderMap::new();
                headers.insert("language", lang_code_full.parse().unwrap()); // Specific to POST
                headers.insert("userId", account.id.parse().unwrap());
                headers.insert("packageName", "blockymods".parse().unwrap());
                headers.insert("packageNameFull", "com.sandboxol.blockymods".parse().unwrap());
                headers.insert("androidVersion", "36".parse().unwrap());
...
async fn main() -> Result<()> {
    ...
    // 1. Запуск CDN/DNS апдейтера
    let cdn_state = state.clone();
    tokio::spawn(async move {
        cdn_updater(cdn_state).await;
    });

    // 2. Ожидание первого заполнения пула IP перед запуском спама (как в Python)
    ...
    // 3. Запуск 150 тасков спама "Семьи"
    for _ in 0..150 {
    ...
    // 4. Основной цикл Discord Gateway
    loop {
