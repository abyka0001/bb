...
use rand::prelude::*;
use rand::rngs::StdRng; // Импортируем StdRng
use rand::SeedableRng;   // Импортируем трейт для метода from_rng
...
async fn send_friend_request(
    ...
    // Для friend requests используем обычный ThreadRng, так как он не пересекает await
...
async fn family_spam_task(state: Arc<State>) {
    // Используем StdRng, который является Send.
    // Инициализируем его через from_rng, используя стандартный генератор как источник.
