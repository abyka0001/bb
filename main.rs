...
use rand::prelude::*;
use rand::rngs::StdRng; // Импорт StdRng (он Send)
use rand::SeedableRng;   // Импорт трейта для метода from_os_rng
...
async fn family_spam_task(state: Arc<State>) {
    // В rand 0.9+ метод from_entropy переименован в from_os_rng.
    // StdRng реализует SeedableRng и является Send (в отличие от ThreadRng), что важно для async.
